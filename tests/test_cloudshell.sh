...
expect "legacy hourly parquet, peak month" legacy_hourly \
  "uv run kion-sizer --s3 s3://cur-bucket/reports/hourly-cur/yearMonth=202606/ --granularity hourly --rds-from-aws --cost --read-footers --detect-accounts"

expect "prefers HOURLY+parquet among reports" multi_report \
  "uv run kion-sizer --s3 s3://hourly-bkt/r/hourly/yearMonth=202606/ --granularity hourly --rds-from-aws --cost --read-footers --detect-accounts"

expect "CUR 2.0 data-exports fallback (BILLING_PERIOD)" data_exports \
  "uv run kion-sizer --s3 s3://cur2-bkt/exports/cur2/data/BILLING_PERIOD=2026-06/ --rds-from-aws --cost --read-footers --detect-accounts"

expect "bucket-name heuristic fallback" bucketname \
  "uv run kion-sizer --s3 s3://my-cur-bucket/cur/yearMonth=202606/ --rds-from-aws --cost --read-footers --detect-accounts"
...
expect "--bucket picks peak month in named bucket" named_bucket \
  "uv run kion-sizer --s3 s3://named-bkt/reports/parquet/yearMonth=202606/ --rds-from-aws --cost --read-footers --detect-accounts" \
  --bucket named-bkt
...
expect "--s3 takes precedence over --bucket" named_bucket \
  "uv run kion-sizer --s3 s3://manual/pfx/ --rds-from-aws --cost --read-footers --detect-accounts" \
  --s3 s3://manual/pfx/ --bucket named-bkt

expect "accounts + json passthrough" passthrough \
  "uv run kion-sizer --s3 s3://cur-bucket/reports/hourly-cur/yearMonth=202606/ --granularity hourly --accounts 150 --json --rds-from-aws --cost --read-footers --detect-accounts" \
  --accounts 150 --json
...
[ "$got" = "uv run kion-sizer --s3 s3://manual/pfx/ --rds-from-aws --cost --read-footers --detect-accounts" ] \
  && ok "--s3 override skips discovery" \
  || bad "--s3 override skips discovery" "uv run kion-sizer --s3 s3://manual/pfx/ --rds-from-aws --cost --read-footers --detect-accounts" "$got"
